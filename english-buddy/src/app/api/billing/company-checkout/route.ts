...
import { MAX_TEAM_SEATS, MIN_TEAM_SEATS, TEAM_PLANS, teamUnitAmount } from "@/lib/licenses";
...
const bodySchema = z.object({
  ...
  // Older clients cached before the annual package existed send no plan.
  plan: z.enum(["program", "annual"]).default("program"),
});
...
export async function POST(request: Request) {
  ...
  const { companyName, email, quantity, plan } = parsed.data;

  const unitAmount = teamUnitAmount(quantity, plan);
  ...
  try {
    ...
    const url = await createTeamCheckout(
      { companyName, buyerEmail: email, quantity, unitAmount, plan, planLabel: TEAM_PLANS[plan].label },
      base
    );
