...
export default async function ProgressPage() {
  ...
  const mistakes = mistakesResult.rows as any[];
  const expressions = expressionsResult.rows as any[];
  const values = Object.keys(labels).map(k => [k, Number(state?.[k] ?? 50)] as const);
  return <main className="shell"><div className="topbar"><div className="brand">Progress</div><span className="chip">CEFR {state?.cefr_level || "—"}</span></div>
